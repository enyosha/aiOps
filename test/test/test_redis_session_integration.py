...
async def test_full_lifecycle():
    ...
    assert redis_session is not None, "会话应该�?Redis �?
    assert redis_session.session_id == session_id, "session_id 应该匹配"
    print("�?会话已保存到 Redis")
    
    # 步骤 5: 添加消息
    print("\n步骤 5: 添加消息...")
    conversation_manager.add_message(session_id, "user", "你好，请介绍一下自�?)
    conversation_manager.add_message(session_id, "assistant", "我是一�?AI 助手，可以帮助您解答问题�?)
    ...
    assert len(reloaded.messages) == 3, f"应该�?3 条消息，实际�?{len(reloaded.messages)} �?
    assert reloaded.messages[0].content == "你好，请介绍一下自�?
    assert reloaded.messages[1].content == "我是一�?AI 助手，可以帮助您解答问题�?
    ...
    assert stats['message_count'] == 3, "消息数量应该�?3"
    assert stats['session_id'] == session_id, "session_id 应该匹配"
    print(f"�?会话统计: {stats['message_count']} 条消�? 持续 {stats['duration_seconds']:.0f} �?)
...
async def test_session_restore():
    ...
    try:
        ...
        print("\n步骤 1: 创建会话并添加消�?..")
        session_id = conversation_manager.create_session()
        conversation_manager.add_message(session_id, "user", "第一轮对�?)
        conversation_manager.add_message(session_id, "assistant", "这是第一轮回�?)
        print(f"�?创建会话: {session_id[:8]}...")
        print("�?添加�?2 条消�?)
        ...
        print(f"�?成功�?Redis 恢复会话: {session_id[:8]}...")
        
        # 验证消息完整�?
        assert len(restored.messages) == 2, f"应该�?2 条消息，实际�?{len(restored.messages)} �?
        assert restored.messages[0].content == "第一轮对�?
        assert restored.messages[1].content == "这是第一轮回�?
        print("�?消息完整性验证通过")
        print("�?会话恢复功能正常")
...
async def test_multiple_sessions():
    ...
    try:
        ...
        for i, sid in enumerate(sessions):
            conversation_manager.add_message(sid, "user", f"会话 {i+1} 的用户消�?)
            conversation_manager.add_message(sid, "assistant", f"会话 {i+1} �?AI 回复")
            print(f"  �?会话 {i+1}: 添加�?2 条消�?)
        
        # 验证所有会话独立存�?
        print("\n步骤 3: 验证会话独立�?..")
        for i, sid in enumerate(sessions):
            session = conversation_manager.get_session(sid)
            assert len(session.messages) == 2, f"会话 {i+1} 应该�?2 条消�?
            assert session.messages[0].content == f"会话 {i+1} 的用户消�?
            assert session.messages[1].content == f"会话 {i+1} �?AI 回复"
            print(f"  �?会话 {i+1}: 数据独立且正�?)
        ...
        print(f"�?Redis 中有 {len(sessions)} 个活跃会�?)
        print("�?多会话并发管理正�?)
...
async def test_session_expiration():
    ...
    try:
        ...
        # 手动修改 last_active �?2 小时�?
        print("\n步骤 2: 模拟会话过期（设置为 2 小时前）...")
        session.last_active = time.time() - 7200  # 2 小时 = 7200 �?
