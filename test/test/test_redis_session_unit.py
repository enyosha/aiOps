...
2. 验证会话创建、消息添加和保存的完整流�?
3. 验证退出时资源清理逻辑
4. 不依赖真�?Redis 服务器，使用 Mock 模拟
...
def test_startup_no_history():
    ...
    # Mock list_recent_sessions 返回空列�?
    with patch.object(conversation_manager, 'list_recent_sessions', return_value=[]):
        ...
        assert len(recent) == 0, "应该返回空列�?
        print("�?list_recent_sessions 返回空列�?)
...
def test_startup_with_history():
    ...
    with patch.object(conversation_manager, 'list_recent_sessions', return_value=mock_sessions):
        ...
        assert len(recent) == 2, "应该返回 2 个会�?
        print(f"�?list_recent_sessions 返回 {len(recent)} 个会�?)
...
async def test_create_session_and_save():
    ...
    try:
        ...
        # 验证 save_session 被调�?
        assert mock_redis_store.save_session.called, "save_session 应该被调�?
        print("�?redis_store.save_session() 被调�?)
...
async def test_add_message_sync_to_redis():
    ...
    try:
        ...
        # 验证 save_session 被调�?
        assert mock_redis_store.save_session.call_count >= 1, "添加消息后应该调�?save_session"
        print("�?添加用户消息后触�?save_session")
        ...
        print("�?添加 AI 回复后再次触�?save_session")
        
        # 验证消息数量
        history = conversation_manager.get_history(session_id)
        assert len(history) == 2, "应该�?2 条消�?
...
async def test_cleanup_resources():
    ...
    try:
        ...
        # 验证 Redis 连接关闭
        assert mock_redis_store.close.called, "Redis 连接应该被关�?
        print("�?Redis 连接已关�?)
        
        # 验证 SSH 隧道关闭
        assert mock_tunnel.close_tunnel.called, "SSH 隧道应该被关�?
        print("�?SSH 隧道已关�?)
