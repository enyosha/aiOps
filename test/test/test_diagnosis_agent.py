"""
测试DiagnosisAgent - 纯LLM驱动的运维诊断
测试场景：Grafana检测到容器异常后自动触发诊断
"""
import asyncio
import sys
import os
...
async def test_diagnosis():
    ...
    if result['status'] == 'success' and result.get('diagnosis'):
        ...
        # 检查点
        checks = [
            ("识别到内存紧张", any(keyword in content for keyword in ["内存", "memory", "231Mi", "可用"])),
            ("基于实时数据分析", data_collected.get('memory') or data_collected.get('logs')),
            ("给出了具体修复建议", any(cmd in content for cmd in ["restart", "update", "kill", "docker", "增加", "扩容"])),
