...
async def test_multi_server_logs():
    ...
    print("="*80)
    print("测试1: 读取后端服务�?(8.130.131.36) 上的 ruoyi-app 容器日志")
    print("="*80)
    try:
    ...
    print("\n" + "="*80)
    print("测试2: 读取前端服务�?(8.146.236.55) 上的 ruoyi-frontend 容器日志")
    print("="*80)
    try:
        result = await read_tool.ainvoke({
    ...
    try:
        ...
        if log_data.get('status') == 'error':
            ...
            print(f"�?未能识别错误，返�?status: {log_data.get('status')}")
