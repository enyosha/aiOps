...
def main():
    ...
    print(f"   密钥: {ssh_key_path if ssh_key_path else '未配置'}")
    
    # 检查密钥文件
    if not ssh_key_path or not os.path.exists(ssh_key_path):
        print(f"\n❌ 错误: 找不到SSH密钥文件: {ssh_key_path}")
        print("💡 请在 .env 文件中正确配置 FRONTEND_SSH_KEY_PATH")
        return
    ...
    try:
        ...
        if container_name in output:
            ...
            print(f"   ⚠️ 容器 {container_name} 未运行或不存在")
        ...
        if container_exists:
            ...
            if volumes:
                print(f"   卷挂载: {volumes[:100]}..." if len(volumes) > 100 else f"   卷挂载: {volumes}")
        else:
            # 容器不存在，使用默认配置
        ...
        print("="*80)
        print("\n💡 下一步:")
        print(f"   1. 在前端页面执行一些操作")
        print(f"   2. 运行: docker logs {container_name} --tail 20")
        print(f"   3. 确认日志时间显示为北京时间 (UTC+8)")
        print()
        
    except FileNotFoundError:
        print(f"\n❌ 错误: 找不到SSH密钥文件: {ssh_key_path}")
        print("💡 请检查 .env 文件中的 FRONTEND_SSH_KEY_PATH 配置")
