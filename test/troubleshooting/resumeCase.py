...
def resume_case01():
  ...
  ruoyi-app:latest"""
    ...
    if http_code.strip() == "200":
        ...
        print(f"   ⚠️  服务状态码: {http_code.strip()},可能需要更多时间启动")
    else:
        print("   ❌ 无法访问服务,请检查容器状态")
...
def resume_case03():
    ...
    if "Backup not found" in output:
        print("   ⚠️  未找到备份文件,可能需要手动恢复配置")
        print("   💡 建议检查容器内是否有备份: docker exec ruoyi-app ls -la /app/ruoyi-admin/src/main/resources/")
    else:
        print("   ✅ 配置文件已恢复")
    ...
    if "maxActive: 20" in config_output or "maxActive:20" in config_output:
        print("   ✅ 连接池配置已恢复为正常值(maxActive=20)")
    else:
        print("   ⚠️  配置可能未正确恢复,当前配置:")
...
def resume_case02():
    ...
    if "Backup not found" in output:
        print("   ⚠️  未找到备份目录,可能需要重新部署应用")
        print(f"   💡 建议检查: docker exec {container_name} ls -la /app/ruoyi-admin/src/main/resources/")
    else:
        print("   ✅ 静态资源目录已恢复")
    ...
    print("   2. 检查浏览器控制台Network标签,确认无404错误")
    print("   3. 验证页面样式和交互功能正常")
    print()
...
def resume_case05():
    ...
    if error and "ERROR" in error:
        print(f"   ⚠️  删除表警告: {error[:100]}")
    else:
        print("   ✅ 测试表已删除")
    
    # 步骤3: 验证CPU使用率
    print("\n3️⃣  检查CPU使用率...")
    time.sleep(5)  # 等待几秒让CPU降下来
...
def resume_case04():
    ...
    if error or "No such file" in output:
        print("   ⚠️  未找到备份文件,可能需要手动恢复配置")
        print(f"   💡 建议检查: ls -la /opt/source/RuoYi/")
    else:
        print("   ✅ 配置文件已恢复")
    ...
    print("   2. 检查容器日志无异常: docker logs ruoyi-app --tail 50")
    print("   3. 监控系统资源使用情况")
    print("   4. 确认与Case03(Linux OOM)的区别")
    print()
