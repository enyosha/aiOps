...
使用场景:
    1. 编写 generateCase.py/resumeCase.py 时确认文件路径
    2. 故障构建失败时排查路径问题
    3. 验证容器内目录结构是否符合预期
...
def check_config_files(container_name):
    ...
    if output.strip():
        ...
        print("   ❌ 未找到 application.yml")
    
    # 查找所有 application*.yml
    ...
    if output.strip():
        ...
        print("   ❌ 未在 /app 目录下找到 application*.yml")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='容器路径诊断工具 - 快速探查容器内部文件路径结构',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python debug_container_paths.py                      # 检查默认容器 ruoyi-app
  python debug_container_paths.py --container mysql    # 检查 MySQL 容器
  python debug_container_paths.py -c ruoyi-app --section config  # 仅检查配置
        """
    )
    ...
    parser.add_argument(
        ...
        default='all',
        help='检查部分: config/directory/logs/static/all (默认: all)'
    )
    ...
    if container_name not in output:
        ...
        print("💡 请先启动容器或使用 'docker ps' 查看可用容器")
        return
    
    print(f"✅ 容器 '{container_name}' 运行中")
    ...
    print("="*80)
    print("\n💡 下一步:")
    print("   1. 将找到的路径应用到 generateCase.py / resumeCase.py")
    print("   2. 如需检查其他容器: python debug_container_paths.py -c <容器名>")
    print()
