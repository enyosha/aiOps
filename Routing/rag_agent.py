...
def debug_print(message: str):
    """调试信息打印并写入日志文件"""
    print(f"DEBUG RAG Agent: {message}")
...
class ModelNode:
    ...
    async def __call__(self, state: AgentState) -> AgentState:
        ...
        # 添加系统提示词来指导模型处理RAG问答
        system_prompt = SystemMessage(content="""你是一个专业的知识库问答助手,专门回答关于Data目录中存储的文档内容。

你的工作流程:
1. 分析用户问题,理解需要查询的知识
2. 调用search_knowledge工具在知识库中检索相关文档
3. 基于检索到的文档片段,准确、简洁地回答用户问题
4. 如果检索结果不相关或为空,如实告知用户"知识库中没有找到相关信息"

重要规则:
- 必须调用search_knowledge工具进行检索,不要凭空编造答案
- 回答时必须基于检索到的文档内容,可以适当总结和重组语言
- 如果工具返回多个相关片段,综合这些信息给出完整回答
- 引用关键信息时可以注明来源文件
- 保持回答的专业性和准确性
- 如果问题超出知识库范围,明确说明
""")
        ...
        debug_print(f"RAG model node called with {len(full_messages)} messages")

        # 动态加载RAG工具
        tools = await self._load_rag_tools()

        # 绑定工具到LLM
        llm_with_tools = self.llm.bind_tools(tools)
        response = await llm_with_tools.ainvoke(full_messages)

        debug_print(f"RAG model response - Type: {type(response)}, Tool calls: {getattr(response, 'tool_calls', 'None')}")
    ...
    async def _load_rag_tools(self):
        ...
        # 获取RAG服务器配置
        rag_config = self.config.get("rag-knowledge")
        if not rag_config or rag_config.get("transport") != "stdio":
            print(" RAG服务器配置未找到或不是stdio类型")
            return tools
        ...
        try:
            ...
            async with stdio_client(server_params) as (read, write):
                async with ClientSession(read, write) as session:
                    ...
                    print(f" 从RAG服务器加载了 {len(loaded_tools)} 个工具")
        except Exception as e:
            print(f" 加载RAG服务器工具失败: {e}")
...
class ToolNode:
    ...
    async def __call__(self, state: AgentState) -> AgentState:
        ...
        debug_print(f"RAG tool node called with {len(messages)} messages, last message: {type(last_message).__name__ if last_message else 'None'}")
        ...
        for tool_call in tool_calls:
            ...
            debug_print(f"Executing RAG tool {tool_name} with args {tool_args}")

            # 执行RAG工具调用
            result_content = await self._execute_rag_tool(tool_name, tool_args)
            ...
            if result_content:
                ...
                tool_results.append({
                    "name": tool_name,
                    "args": tool_args,
                    "error": f"Failed to execute RAG tool {tool_name}"
                })
    ...
    async def _execute_rag_tool(self, tool_name, tool_args):
        """执行RAG工具调用"""
        rag_config = self.config.get("rag-knowledge")

        if not rag_config or rag_config.get("transport") != "stdio":
            print(" RAG服务器配置未找到或不是stdio类型")
            return f"RAG server not configured: {tool_name}"
        ...
        if not os.path.exists(script_path):
            print(f" 服务器脚本不存在: {script_path}")
            return f"RAG server script not found: {script_path}"
            ...
            async with stdio_client(server_params) as (read, write):
                async with ClientSession(read, write) as session:
                    ...
                    if target_tool:
                        ...
                        return f"RAG tool {tool_name} not found in server"
        except Exception as e:
            print(f"RAG工具执行错误：{e}")
            return f"Error executing RAG tool {tool_name}: {str(e)}"
